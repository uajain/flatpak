  ...
  if (flatpak_dir_use_system_helper (self, NULL))
    {
      ...
      if (no_pull)
      ...
      else
        {
          ...
          g_autofree gchar *repo_tmp_cache_dir = NULL;
          g_autoptr(OstreeRepo) child_repo = NULL;
          g_auto(GLnxLockFile) child_repo_lock = { 0, };
          g_autoptr(GError) local_error = NULL;
          ...
          if (!flatpak_dir_system_helper_call_create_pull_cache_dir (self, ref, opt_commit,
                                                                     ...
                                                                     &repo_tmp_cache_dir, cancellable, &local_error))
            {
              if (g_error_matches (local_error, G_DBUS_ERROR, G_DBUS_ERROR_UNKNOWN_METHOD) ||
                ...
                g_debug ("Cannot create pull cache dir in repo/tmp: %s", local_error->message);
              else
                g_warning ("Cannot create pull cache dir in repo/tmp: %s", local_error->message);
            }
          else
            {
              ...
              if (child_repo == NULL)
                {
                  g_warning ("Cannot create child repo at path %s: %s", repo_tmp_cache_dir, local_error->message);
