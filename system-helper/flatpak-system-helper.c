...
G_DEFINE_AUTOPTR_CLEANUP_FUNC (FlatpakSystemHelperOngoingPull, system_helper_ongoing_pull_free)

static void
name_vanished_callback (GDBusConnection *connection, const gchar *name, gpointer user_data)
{
  system_helper_ongoing_pull_free ((FlatpakSystemHelperOngoingPull *) user_data);
}
                              ...
                              const gchar           *arg_installation)
{
  ...
  if (bindfs_path == NULL)
  ...
  else
    {
      ...
      g_autoptr(FlatpakSystemHelperOngoingPull) pull_tmp = NULL;
      ...
      pull_tmp->root_dir_name = g_steal_pointer (&root_dir);

      new_pull = g_steal_pointer (&pull_tmp);
      G_LOCK (ongoing_pulls);
